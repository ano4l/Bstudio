...
const portfolioSource=path.join(root,'assets','service-portfolio'),portfolioDist=path.join(dist,'assets','service-portfolio');fs.mkdirSync(portfolioDist,{recursive:true});for(const asset of fs.readdirSync(portfolioSource))fs.copyFileSync(path.join(portfolioSource,asset),path.join(portfolioDist,asset));
